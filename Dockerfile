...
# Dockerfile内で指定した命令を実行する ...RUN, COPY, ADD, ENTRYPOUNT, CMD
# 作業ディレクトリを定義
# コンテナ /app/Railsアプリ
WORKDIR ${HOME}

# ホスト側(PCのファイルをコンテナにコピー
# COPY コピー元(ホスト) コピー先(コンテナ)
# Gemfile* ... Gemfileから始まるファイルを全指定(Gemfile, Gemfile.lock)
# コピー元（ホスト） ... Dockerfileがあるディレクトリ以下を指定(api) .../ NG
# コピー先（コンテナ） ... 絶対パス or 相対パス(./ ... カレントディレクトリ)
COPY Gemfile* ./

# apk ... Alpile Linuxのコマンド

    # apk update = パッケージの最新リストを取得
RUN apk update && \
    # apk upgrafe = インストールパッケージを最新のものに
    apk upgrade && \
...
# .とは ... Dockerfileがあるディレクトリ全てのファイル（サブディレクトリも含む）
COPY . ./

# コンテナ内で実行したいコマンドを定義
# -b ... バインド。プロセスを指定したip(0.0.0.0)アドレスに紐付け（バインド）する
# CMD ["rails", "server", "-b", "0.0.0.0"]
# →docker-compose.ymlのコマンドで起動させるので、コメントアウトにした
